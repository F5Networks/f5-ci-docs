...
The example F5 resource JSON blob shown below defines the ``f5.http`` iApp. The ``iappVariables`` configuration parameters correspond to fields in the `iApp template <https://support.f5.com/kb/en-us/products/big-ip_ltm/manuals/product/bigip_iapps_developer_11_0_0/2.html#unique_1762445433>`_ 's presentation section. You can :ref:`create iApp variables <create-iapp-variables>` for any built-in or custom iApp.
...
#. Remove the ConfigMap from the Kubernetes API server to delete the corresponding objects from the BIG-IP.
    ...
    .. code-block:: bash
        ...
        user@k8s-master:~$ kubectl delete configmap k8s.f5http --namespace k8petstore
