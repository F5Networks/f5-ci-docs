...
There are a number of options when it comes to connecting your BIG-IP device (platform or Virtual Edition) to a Kubernetes cluster network.
How you choose to integrate your BIG-IP device into the cluster network -- and the framework you use -- impacts how |kctlr| manages objects on the BIG-IP system.

See :ref:`The F5 Integration and Kubernetes cluster networks <k8s-bigip-networks>` for more information.
...
Find out more about the :ref:`F5 OpenShift Origin Integration <openshift-home>` and how to :ref:`replace the OpenShift F5 Router <replace-f5-router>` with the |kctlr|.

Monitors and Node Health
------------------------

If your Services use type :ref:`NodePort`, the |kctlr| has no visibility into Kubernetes node health.
This means that **all pool members on a down Kubernetes node will remain active even if the node itself is unavailable**.
